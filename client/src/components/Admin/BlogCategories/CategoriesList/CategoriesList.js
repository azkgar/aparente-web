...
export default function CategoriesList(props) {
    ...
    const activateCategory = (categories, status) => {
        const accessToken = getAccessTokenApi();
        activateCategoryApi(accessToken, categories._id, status).then(
            response => {
    ...
    const deleteCategory = (categories) => {
        ...
        confirm({
            ...
            cancelText: "Cancelar",
            onOk() {
                deleteCategoryApi(accessToken,categories._id).then(response => {
                    notification["success"]({message: response});
    ...
    return(
        <div className = "categories-list">
            <div className = "categories-list__header">
                <Button type = "primary" onClick = {addCategoryModal}>Crear nueva categoría</Button>
            </div>
...
function CategoryItem(props) {
    ...
    return(
        <List.Item
            actions = {[
                <Switch defaultChecked = {item.active} onChange = {e => activateCategory(item, e)} />,
                <Button type = "primary" onClick = {() => editCategoryModal(item)}>
                    <EditOutlined />
                </Button>,
